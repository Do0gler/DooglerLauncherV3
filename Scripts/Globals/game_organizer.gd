...
func get_organized_games() -> Dictionary:
	if current_grouping == "none":
		...
		var sorted_games = SettingsManager.manager.games_library.duplicate()
...
func get_grouped_and_sorted_games() -> Dictionary:
	...
	for key in grouped:
		grouped[key].sort_custom(func(a, b): return compare_games(a, b))
...
func set_sorting(sorting_type: String) -> void:
