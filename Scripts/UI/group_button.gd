...
func add_category_option(label: String, id := -1) -> void:
	...
	menu_popup.set_item_icon(id, enabled_icon)
	menu_popup.set_item_icon_max_width(id, 25)

func _on_item_selected(index: int) -> void:
	match index:
		0:
			GameOrganizer.set_grouping("favorited")
		1:
			GameOrganizer.set_grouping("engine")
		2:
			GameOrganizer.set_grouping("status")
		3:
			GameOrganizer.set_grouping("none")
